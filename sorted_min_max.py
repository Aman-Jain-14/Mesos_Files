...
import time
rootdir = '/home/aman/Desktop/Results2/PSDSF_A_Fine/'
    ...
    for q in range (0,2):
        ...
        if(arr2[q] == 'try1' or arr2[q] == 'try2'):
            ...
            while(i<len(lines)):
                ...
                l1.append(s_t)
                l2.append(e_t)
                temp = type_of_task+'\t'+tid+'\t'+s_t+'\t'+e_t+'\t'+total+'\n'
                dict[s_t] = temp
            ...
            for key in sorted(dict.iterkeys()):
                count += 1
                f1.write(str(count) + '. ' + dict[key])
                # print "%s: %s" % (key, mydict[key])

            f1.write('\n')
            if(arr2[q] == 'try2'):
                f1.write('Min Time = '+str(min(l1))+'\n')
                f1.write('Max Time = '+str(max(l2))+'\n')
                tdelta = datetime.strptime(max(l2), FMT) - datetime.strptime(min(l1), FMT)
                f1.write('Time Difference = '+str(tdelta))
            f1.close()
